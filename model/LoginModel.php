...
function checkLoginUser($username, $password){
    //$username vaf $password la du lieu nhap tu form login
    ...
    $dataUser = [];//mang rong chua ttin nguoi dung
    if ($statement){
        //kie  tra cac tham so truyen vao sql
        $statement ->bindParam(':user', $username, PDO::PARAM_STR);
        $statement ->bindParam(':pass', $password, PDO::PARAM_STR);
        //thuc thi cau lenh
        if($statement->execute()){
            // kiem tra xem truy van sql co du lie tra ve hay khong
    ...
    return $dataUser; // tra ve du lieu chua ttin nguoi dung
}